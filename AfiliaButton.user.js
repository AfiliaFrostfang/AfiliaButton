...
(async function() {
    ...
    async function loadState() {
        const parsedState = await idbGet("AfiliaState");
        if (parsedState) {
    ...
    if(!sessionStorage.aVehicleTypesNew || JSON.parse(sessionStorage.aVehicleTypesNew).lastUpdate < (new Date().getTime() - 4 * 500 * 60)) {
        try {
            ...
            const response = await fetch("https://afiliafrostfang.github.io/AfiliaButton/vehicletype.json");
        ...
        for (var i in allianceMissions) {
            ...
            intoTable +=
                `<tr id="tr_${e.id}" class="alert alert-info">
                   <td class="col">
                     <a class="lightbox-open" href="/missions/${e.id}">${e.name}</a>
                   </td>
   ...
   $("body").on("click", "#chilloutArea", function() {
    ...
    $("body").on("click", "#AfiliaPreferences", async function() {
        ...
        $("#AfiliaModalBody").html(`
            ...
            <br><br>
            <label for="AfiliaTimeout">Timeout (ms): </label><input type="text" class="form-control form-control-sm" value="${timeout}" id="AfiliaTimeout" style="width:5em;height:22px;display:inline"><span> ms</span>
            <br><br>
